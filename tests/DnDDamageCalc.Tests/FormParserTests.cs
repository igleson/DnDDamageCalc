...
public class FormParserTests
{
    ...
    [Fact]
    public void Parse_BoonOfCombatProwess_ParsedCorrectly()
    {
        var form = CreateForm(new Dictionary<string, string>
        {
            ["characterName"] = "Test",
            ["level[0].number"] = "1",
            ["level[0].resources.hasBoonOfCombatProwess"] = "true"
        });

        var character = FormParser.Parse(form);

        Assert.True(character.Levels[0].Resources.HasBoonOfCombatProwess);
    }
